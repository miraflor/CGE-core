...
for source in sorted(DOCS.rglob("*.md")):
    ...
    for raw in doc_role.findall(text):
        target = raw.rsplit("<", 1)[1][:-1].strip() if "<" in raw and raw.endswith(">") else raw
        if "://" in target:
            continue
        ...
        if not candidate.is_file():
            errors.append(f"{source.relative_to(DOCS)}: broken doc target {raw!r}")

for source in [DOCS / "MODEL.md", *sorted((DOCS / "theory").glob("*.md"))]:
    text = source.read_text(encoding="utf-8")
    if r"\[" in text or r"\]" in text or r"\(" in text or r"\)" in text:
        errors.append(f"{source.relative_to(DOCS)} contains backslash math delimiters.")
...
extra = config.get("sphinx", {}).get("extra_extensions", []) if isinstance(config, dict) else []
if mermaid_found and "sphinxcontrib.mermaid" not in extra:
    errors.append("Mermaid is present but sphinxcontrib.mermaid is not enabled.")
...
print(f"Documentation source checks passed: {len(toc_files)} TOC pages, {len(all_md)} Markdown pages.")
